...
export default class Loginpage extends Component {
  ...
  handleRequest = async e => {
    ...
    const user = {
      secretFarmerName: encrypt.encrypt(secretUserName),
      farmerName: username,
      farmerMobile: mobileNumber,
      farmerSecret: encrypt.hash(password),
    };
    const response = await axios.post(`${URL.url}/login`, user);
    console.log(response);
    if (response.data.code === 200 && response.data.result.length === 1) {
      const cookie = new Cookie();
      cookie.set('farmerSecretUserName', encrypt.encrypt(secretUserName));
      cookie.set('farmerUserName', username);
      cookie.set('farmerMobileNumber', mobileNumber);
      document.location.href = 'http://localhost:11002/';
    } else {
      document.location.href = 'http://localhost:11002/login';
    }
  };
            ...
            <div className="card card1">
              <div className="row justify-content-center my-auto">
                <div className="col-md-8 col-10 my-5">
                  ...
                  <div className="row justify-content-center my-3 px-3">
                    <a href="http://localhost:11002/signup">Sign up</a>
                  </div>
                </div>
              </div>
            </div>
