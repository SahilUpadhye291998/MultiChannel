...
function generateChannelArtifacts(){
  ...
  echo "###############################################################################"
  set -x
  configtxgen -profile LogisticsChannel -outputCreateChannelTx ./channel-artifacts/logisticschannel.tx -channelID $CHANNEL_NAME_LOGISTICS
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate channel configuration transaction..."
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile LogisticsChannel -outputAnchorPeersUpdate ./channel-artifacts/Org4MSPanchors.tx -channelID $CHANNEL_NAME_LOGISTICS -asOrg Org4MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for Org3MSP..."
...
function networkUp(){
    ...
    if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
      ...
      # docker exec cli_supplier_farmer /bin/sh -c "scripts/network_supplier_farmer.sh"
...
CHANNEL_NAME_LOGISTICS="logistics"
LANGUAGE=javascript
CRYPTO="cryptogen"
export VERBOSE=true
NO_CHAINCODE=false
